...
data class AndLeft(override val pos: Int) : PositionDependendRule(pos) {

    override fun applicable(sequent: Sequent): Boolean = RuleUtils.applicableInAntecedent<Binary, BinarySymbol>(sequent, pos, BinarySymbol.AND)

    override fun apply(sequent: Sequent): List<Sequent> {
        val (sequent, pivot) = RuleUtils.extractPivot<Binary>(sequent, pos)
        return listOf(sequent.withPremise(pivot.leftFormula)
